...
def plotHeapUsage(appName, title, gcActivities):
    ...
    fig, axes = plt.subplots(nrows=4, ncols=1, sharey=False, sharex=True, figsize=(6.3, 7.3))

    plt.rc('font', family='Helvetica', size=10)

    plt.subplots_adjust(left=0.11, bottom=0.04, right=0.96, top=0.94,
    ...
    freedObjSizeLine = axes[1].plot(freedObjSize[0], freedObjSize[1], 'o-', linewidth=1.5, markersize=2, label='Freed Non-large Object Size')
    axes[1].legend(frameon=False)

    freedLargeObjSize = heapUsage.getFreedLargeObjSizesAndTime()
    freedLargeObjSizeLine = axes[2].plot(freedLargeObjSize[0], freedLargeObjSize[1], 'o-', linewidth=1.5, markersize=2, label='Freed Large Object Size')
    freedAvgLargeObjSize = heapUsage.getFreedAvgLargeObjSizesAndTime()
    freedAvgLargeObjSizeLine = axes[2].bar(freedAvgLargeObjSize[0], freedAvgLargeObjSize[1], 0.5, color='red', label='Freed Average Large Object Size')

    axes[2].legend(frameon=False)

    gcPauseAndTime1 = heapUsage.getGCPauseAndTime("Background partial concurrent mark sweep GC")
    #if (len(gcPauseAndTime1) != 0):
    partital = axes[3].bar(gcPauseAndTime1[0], gcPauseAndTime1[1], 0.5, label='Background partial CMS')
    ...
    if (title != "Alexnet GC Metrics"):
        strict = axes[3].bar(gcPauseAndTime2[0], gcPauseAndTime2[1], 0.3, label='Background stricky CMS')
